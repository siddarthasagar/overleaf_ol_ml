...
Randomized Optimization in Part 1 operates on a \emph{flattened} vector of only the final output-layer parameters (weight matrix + bias vector). This is \textbf{intentional}: exposing all $\sim$10{,}000 architecture parameters would make the search dimensionality intractable within a 50{,}000 function evaluation budget. Restricting to the last 2 parameter tensors (output-layer weight and bias) limits the search to exactly 202 parameters for Adult ($100\times2 + 2$) and 808 for Wine ($100\times8 + 8$), keeping RO tractable.
...
All Part 3 experiments run on Adult Income only (per assignment scope restrictions), using standard Adam with the best Part 2 hyperparameters ($\text{lr} = 10^{-3}$, up to 20 epochs, approximately 4{,}240 gradient steps). Regularization strategies compared: no regularization (baseline), L2 weight decay ($\lambda \in \{10^{-4}, 10^{-3}, 10^{-2}\}$; $\lambda = 10^{-3}$ selected by minimum validation loss), Dropout ($p \in \{0.1, 0.2, 0.3\}$; $p = 0.1$ selected, applied after the single hidden layer, i.e., between the hidden-layer output and the pre-softmax logit computation)~\cite{srivastava2014}, Gaussian input noise ($\sigma_{\text{noise}} \in \{0.01, 0.05, 0.1\}$; $\sigma = 0.1$ selected, training-only), label smoothing ($\epsilon \in \{0.01, 0.05, 0.1\}$; $\epsilon = 0.01$ selected, applied to target labels during cross-entropy computation), and an Early Stopping condition (patience $\in \{3, 5, 7\}$ epochs; patience = 3 selected by minimum validation loss, fixed-budget fairness mode: the early-stopping run is allowed to complete the full gradient budget regardless, so the comparison is budget-matched)~\cite{prechelt1998}. All sweeps used a small 3-value grid and selected the setting with the lowest validation loss. The best combination uses Early Stopping (patience=3) + L2 ($\lambda = 10^{-3}$), selected from exhaustive testing of 26 combinations of 2+ regularizers based on minimum validation loss. This 2-regularizer combination outperformed all other tested combinations including those with 3, 4, or 5 regularizers.
