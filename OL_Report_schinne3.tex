...
\begin{figure}[!htbp]
    ...
    \end{subfigure}
    \caption{Part 1: RO convergence trajectories (val loss vs function evaluations, mean $\pm$ std across 10 seeds). \textbf{Takeaway:} On Adult, SA consistently reaches the lowest plateau, while RHC exhibits periodic loss spikes at restart boundaries. On Wine, GA achieves the best final val loss --- confirming the EDA prediction that population-based search advantages single-state methods on overlapping-class loss surfaces.}
